...
const NUM_BLOBS = isMobile ? 150 : isTablet ? 200 : 300;
const BASE_GRAVITY = isMobile ? 0.08 : isTablet ? 0.06 : 0.05;
const BLOB_DAMPING = 0.9;
const MOUSE_FORCE = 10;
const MOUSE_RANGE = 250;
const SCROLL_FORCE = 0.5;
const BLOB_COLOR = [0.0, 0.0, 0.0];
...
class Blob {
	constructor() {
		...
		this.vx = rand(-1, 1);
		this.vy = rand(-1, 1);
		this.r = rand(0.1, isMobile ? 50 : isTablet ? 40 : 30);
	}
	update() {
		...
		else if (this.x + this.r > width) { this.x = width - this.r; this.vx *= -0.5; }

		if (this.y - this.r < 0) { this.y = this.r; this.vy *= -0.5; }
		else if (this.y + this.r > height) { this.y = height - this.r; this.vy *= -0.5; }
...
for (let i = 0; i < NUM_BLOBS; i++) blobs.push(new Blob());
